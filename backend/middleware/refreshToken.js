...
const refreshToken =  async (req, res, next)=>{
    ...
    if (!user) return res.status(401).json({ error : 'Invalid refresh token' });//wrong refresh token => logout (delete refreshToken from Db)
    jwt.verify(
        oldRefreshToken,
        process.env.REFRESH_TOKEN_SECRET,
        async (err) => {
            if (err) {
               return res.status(401).json({ error : 'Invalid refresh token' }); // fires when refreshToken has expired => logout
            }
